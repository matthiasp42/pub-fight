...
app.post('/api/confirm-victory', requireSession, requireGame, (req: GameRequest, res: Response) => {
  const { playerId } = req.body;
  const { gameState } = req.game!;
  ...
  const allConfirmed = Object.keys(gameState.players).every(
    id => gameState.victoryConfirmations.includes(id)
  );
